...
;; Evil Collection for better defaults across modes
...
** VTerm Setup (VSCode-like Terminal)
#+begin_src emacs-lisp
(use-package vterm
  :config
...
** Split Terminal Function
#+begin_src emacs-lisp
(defun my/split-terminal ()
  "Open terminal in bottom split."
  (interactive)
  (let ((term-buffer (multi-vterm)))
    ;; Split window below and resize to ~30% height
...
(global-set-key (kbd "C-`") 'my/split-terminal)
#+end_src
...
(defun my/toggle-vterm ()
  ...
  (interactive)
  (let ((term-window (get-buffer-window "*vterm*" t)))
    (if term-window
